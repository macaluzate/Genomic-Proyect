...
lista_secuencias = list(SeqIO.parse("pr03.fasta", "fasta"))
#para acceder a cada secuencia se hace por medio de un ciclo:

def matriz_puntuacion(lista_secuencias):
  ...
  for secuencia in range(len(lista_secuencias)-1):
    ...
    for i in range(1,fila):
        ...
        for j in range(1,columna):
            ...
            if lista_secuencias[0][i-1] == lista_secuencias[secuencia+1][j-1]:###buscar otro metodo(numpy)
                coincidencia = 2
            else:
                coincidencia = -1
    ...
    id = lista_secuencias[secuencia+1].id
    ...
    return [coef_max,coef_max_posicion,sec_pos,matrix_max,coef_simil]
...
def matriz_recorrido(lista_secuencias, matrix_max, sec_pos):
    ...
    for i in range(1,len(matrix_max)):
        for j in range(1,len(matrix_max[0])):
            ...
            if lista_secuencias[0][i-1] == lista_secuencias[sec_pos][j-1]:
                coincidencia = 2
            else:
                coincidencia = -1
            ##se calcula la relación de recurrencia que indica el algoritmo de Smith–Waterman
...
def resultados(record, matrix2ruta, coef_max_posicion, coef_simil):
    ...
    while matrix2ruta[max_posicion1[0]][max_posicion1[1]] != 0:
        if matrix2ruta[max_posicion1[0]][max_posicion1[1]]== 1:
            principal=principal+lista_secuencias[0][max_posicion1[0]-1]
            comparacion=comparacion+lista_secuencias[sec_pos][max_posicion1[1]-1]
...
if __name__=="__main__":
  datos_puntuacion=matriz_puntuacion(lista_secuencias)
coef_max=datos_puntuacion[0]
coef_max_posicion=datos_puntuacion[1]
sec_pos=datos_puntuacion[2]
matrix_max=datos_puntuacion[3]
coef_simil=datos_puntuacion[4]
matr = matriz_recorrido(lista_secuencias, matrix_max, sec_pos)
resultados(lista_secuencias, matr, coef_max_posicion, coef_simil)